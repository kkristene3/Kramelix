...
A user can have the LLM detect their emotion through text or voice. Modes can be changed using a toggle button.
To run the voice-based model, the local emotion-detection server must be deployed first.
...
2. Copy the IPv4 address (or IPv6 address if applicable) of your LAN or Ethernet.
3. Follow the instructions in: [SETUP.md](Kramelix_app/emotion_training/SETUP.md#file-setup).
...
- Use the pink toggle button in the top right of the app to switch between the LLM using text to detect emotion and the LLM using voice to detect emotion.

**Basic Communication**
- Use the LLM to talk about your day, vent concerns, or get advice. Talk to it however you want!

**Tasks to Ask the LLM**
- Play Music 🎵: ask the LLM to play a song given the song title, the artist name, or to randomly play a song based on a music genre.
...
- Call a Contact ☎️: ask the LLM to call a specific person from your contacts or any number (_Note: emulators cannot be used for this_).

## ❗ Troubleshooting Builds / GitIgnore Information

-   To completely reset the project build, delete the local instances of `/Kramelix_app/.gradle`, `/Kramelix_app/build`, `/Kramelix_app/app/build`, and `/Kramelix_app/app/.cxx`. Afterwards, start from step 7 of the Installation/Setup section above, and these folders should rebuild.
